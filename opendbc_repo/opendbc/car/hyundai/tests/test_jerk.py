...
@pytest.mark.parametrize("camera_scc", [False, True])
def test_canfd_cruise_unavailable_blocks_regular_acc_control(camera_scc):
  class FakePacker:
    @staticmethod
    def make_can_msg(name, bus, values):
      return name, bus, values.copy()
...
def test_canfd_tcs_braking_signals_remain_blocked_during_auto_hold(brake_hold_active):
  ...
  assert values["DriverBraking"] == 0
  assert values["DriverBrakingLowSens"] == 0
  assert values["NEW_SIGNAL_20"] == 0
  assert values["NEW_SIGNAL_11"] == 0
